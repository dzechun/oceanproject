...
public class SrmDeliveryAppointController {
    ...
    @PostMapping("/add")
    public ResponseEntity add(@ApiParam(value = "必传：",required = true)@RequestBody @Validated SrmDeliveryAppointDto srmDeliveryAppointDto) {
    ...
    try {
        // 导出操作
        EasyPoiUtils.exportExcel(list, "导出信息", "SrmDeliveryAppoint信息", SrmDeliveryAppointDto.class, "SrmDeliveryAppoint.xls", response);
        } catch (Exception e) {
