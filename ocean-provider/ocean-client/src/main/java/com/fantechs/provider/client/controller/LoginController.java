...
public class LoginController {
    ...
    public ResponseEntity<List<SmtEquipmentDto>> login(@RequestBody SmtClientManage smtClientManage, HttpServletRequest request) {
        List<SmtEquipmentDto> smtEquipmentDtos = loginServiceImpl.login(smtClientManage, request);
        if (StringUtils.isEmpty(smtEquipmentDtos)){
