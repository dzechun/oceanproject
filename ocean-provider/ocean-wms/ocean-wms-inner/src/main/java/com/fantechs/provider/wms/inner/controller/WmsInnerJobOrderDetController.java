...
@RestController
@Api(tags = "上架作业明细")
@RequestMapping("/wmsInnerJobOrderDet")
@Validated
public class WmsInnerJobOrderDetController {

    @Resource
    private WmsInnerJobOrderDetService wmsInPutawayOrderDetService;
    @Resource
    ...
    public ResponseEntity add(@ApiParam(value = "必传：",required = true)@RequestBody @Validated WmsInnerJobOrderDet wmsInPutawayOrderDet) {
        return ControllerUtil.returnCRUD(wmsInPutawayOrderDetService.save(wmsInPutawayOrderDet));
    }

    @ApiOperation("删除")
    @PostMapping("/delete")
    public ResponseEntity delete(@ApiParam(value = "对象ID列表，多个逗号分隔",required = true) @RequestParam @NotBlank(message="ids不能为空") String ids) {
        return ControllerUtil.returnCRUD(wmsInPutawayOrderDetService.batchDelete(ids));
    }
    ...
    public ResponseEntity update(@ApiParam(value = "对象，Id必传",required = true)@RequestBody @Validated(value= WmsInnerJobOrderDet.update.class) WmsInnerJobOrderDet wmsInPutawayOrderDet) {
        return ControllerUtil.returnCRUD(wmsInPutawayOrderDetService.update(wmsInPutawayOrderDet));
    }
    ...
    public ResponseEntity<WmsInnerJobOrderDet> detail(@ApiParam(value = "ID",required = true)@RequestParam  @NotNull(message="id不能为空") Long id) {
        WmsInnerJobOrderDet wmsInPutawayOrderDet = wmsInPutawayOrderDetService.selectByKey(id);
    ...
    List<WmsInnerJobOrderDetDto> list = wmsInPutawayOrderDetService.findList(searchWmsInPutawayOrderDet);
        ...
        // 自定义导出操作
        EasyPoiUtils.customExportExcel(list, customExportParamList, "导出信息", "WmsInPutawayOrderDet信息", "WmsInPutawayOrderDet.xls", response);
    }
