            ...
            for (PDAWmsInnerDirectTransferOrderDetDto det : dto.getPdaWmsInnerDirectTransferOrderDetDtos()) {
                ...
                for (WmsInnerMaterialBarcode barcode : wmsInnerMaterialBarcodes) {
                    ...
                    if (det.getBarcodeType() == 2) {
                        map.put("colorBoxCode", barcode.getColorBoxCode());
                        List<WmsInnerMaterialBarcodeDto> list = wmsInnerMaterialBarcodeMapper.findList(map);
                        for (WmsInnerMaterialBarcodeDto dtos : list) {
                            if (StringUtils.isNotEmpty(dtos.getCartonCode()) || StringUtils.isNotEmpty(dtos.getPalletCode()))
                                throw new BizErrorException(ErrorCodeEnum.OPT20012003.getCode(), "调拨出库需以最大单位进行，如需单独移动彩盒码，请先进行拆分操作");
                        }
                    }
                    if (det.getBarcodeType() == 3) {
                        map.put("cartonCode",barcode.getCartonCode());
                        List<WmsInnerMaterialBarcodeDto> list = wmsInnerMaterialBarcodeMapper.findList(map);
                        for(WmsInnerMaterialBarcodeDto dtos : list ){
                            if(StringUtils.isNotEmpty(dtos.getPalletCode()))
