 ...
 */
@Service
public class EamHtEquipmentStationServiceImpl extends BaseService<EamHtEquipmentStation> implements EamHtEquipmentStationService {

    @Resource
    private EamHtEquipmentStationMapper eamHtEquipmentStationMapper;

    @Override
    public List<EamHtEquipmentStationDto> findHtList(Map<String, Object> map) {
        SysUser user = CurrentUserInfoUtils.getCurrentUserInfo();
        if (StringUtils.isEmpty(user)) {
            throw new BizErrorException(ErrorCodeEnum.UAC10011039);
        }
        map.put("orgId", user.getOrganizationId());
        return eamHtEquipmentStationMapper.findList(map);
    }
}
