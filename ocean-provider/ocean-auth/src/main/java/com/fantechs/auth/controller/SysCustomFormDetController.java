...
public class SysCustomFormDetController {
    ...
    public ResponseEntity batchUpdate(@ApiParam(value = "明细集合", required = true) @RequestBody List<SysCustomFormDet> list) {
        return ControllerUtil.returnCRUD(sysCustomFormDetService.batchUpdate(list));
    }
    ...
    @ApiOperation("自定义导出参数列表")
    @PostMapping("/findCustomExportParamList")
    public ResponseEntity<List<SysCustomExportDTO>> findCustomFormDetList(@ApiParam(value = "路由地址") @RequestParam @NotNull(message = "fromRout不能为空") String fromRout) {
        Page<Object> page = PageHelper.startPage(1, 99999);
        HashMap<String, Object> map = new HashMap<>();
        map.put("fromRout", fromRout);
        List<SysCustomExportDTO> list = sysCustomFormDetService.findCustomExportParamList(map);
        return ControllerUtil.returnDataSuccess(list, (int) page.getTotal());
    }
