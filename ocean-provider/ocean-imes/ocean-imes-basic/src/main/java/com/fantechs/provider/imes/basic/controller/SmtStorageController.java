...
public class SmtStorageController {
    ...
    @PostMapping("/add")
    public ResponseEntity add(@ApiParam(value = "必传：",required = true)@RequestBody SmtStorage storage) {
        return ControllerUtil.returnCRUD(smtStorageService.insert(storage));
    }
    ...
    public ResponseEntity delete(@ApiParam(value = "对象ID列表，多个逗号分隔",required = true) @RequestParam String ids) {
        ...
        return ControllerUtil.returnCRUD(smtStorageService.batchDel(ids));
    }

    @ApiOperation("修改")
    @PostMapping("/update")
    public ResponseEntity update(@ApiParam(value = "对象，Id必传",required = true)@RequestBody SmtStorage storage) {
        if(StringUtils.isEmpty(storage.getStorageId()
        )){
            return ControllerUtil.returnFailByParameError();
        }
        return ControllerUtil.returnCRUD(smtStorageService.updateById(storage));
    }

    @ApiOperation("获取详情")
    @PostMapping("/detail")
    public ResponseEntity<SmtStorage> detail(@ApiParam(value = "ID",required = true)@RequestParam Long id) {
    ...
    public void exportExcel(HttpServletResponse response, @ApiParam(value = "查询对象")@RequestBody SearchSmtStorage searchSmtStorage){
        List<SmtStorage> list = smtStorageService.findList(searchSmtStorage);
        try {
            // 导出操作
            EasyPoiUtils.exportExcel(list, "导出储位信息", "储位信息", SmtWarehouse.class, "储位信息.xls", response);
        } catch (Exception e) {
