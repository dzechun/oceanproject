...
public class SmtProductBomController {
    ...
    public ResponseEntity<List<SmtProductBomDto>> findList(@ApiParam(value = "查询对象") @RequestBody SearchSmtProductBom searchSmtProductBom) {
        Page<Object> page = PageHelper.startPage(searchSmtProductBom.getStartPage(), searchSmtProductBom.getPageSize());
        List<SmtProductBomDto> list = smtProductBomService.findList(ControllerUtil.dynamicConditionByEntity(searchSmtProductBom));
    ...
    @RequestBody(required = false) SearchSmtProductBom searchSmtProductBom) {
        ...
        try {
            // 导出操作
            EasyPoiUtils.exportExcel(list, "导出产品BOM信息", "产品BOM信息", SmtProductBomDto.class, "产品BOM信息.xls", response);
        } catch (Exception e) {
