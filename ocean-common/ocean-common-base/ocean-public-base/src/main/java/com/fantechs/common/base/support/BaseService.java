...
package com.fantechs.common.base.support;
...
public abstract class BaseService<T> implements IService<T> {
	...
	public int batchSave(List<T> list) {
			int count = mapper.insertList(list);
		return count;
	}
	...
	public int batchDelete(List<T> list) {
		...
		for (T record : list) {
			...
			if (count < 1) {
				logger.error(ErrorCodeEnum.OPT20012000.getMsg());
				throw new BizErrorException(ErrorCodeEnum.OPT20012000.getMsg());
			}
			result += count;
		}
		return result;
