...
/**
 * 销退入库单明细履历表
 * om_ht_sales_return_order_det
 * @author mr.lei
 * @date 2021-06-21 15:03:29
 */
@Data
@Table(name = "om_ht_sales_return_order_det")
public class OmHtSalesReturnOrderDet extends ValidGroup implements Serializable {
    /**
     * 销退入库单明细履历表ID
     */
    @ApiModelProperty(name="htSalesReturnOrderDetId",value = "销退入库单明细履历表ID")
    @Excel(name = "销退入库单明细履历表ID", height = 20, width = 30,orderNum="") 
    @Id
    @Column(name = "ht_sales_return_order_det_id")
    private Long htSalesReturnOrderDetId;
