...
/**
 * 销售编码关联同包装编码履历表
 * om_ht_sales_code_re_spc
 * @author bgkun
 * @date 2021-10-15 10:14:01
 */
@Data
@Table(name = "om_ht_sales_code_re_spc")
public class OmHtSalesCodeReSpc extends ValidGroup implements Serializable {
    /**
     * 销售编码关联同包装履历表ID
     */
    @ApiModelProperty(name="htSalesCodeReSpcId",value = "销售编码关联同包装履历表ID")
    @Excel(name = "销售编码关联同包装履历表ID", height = 20, width = 30,orderNum="") 
    @Id
    @Column(name = "ht_sales_code_re_spc_id")
    private Long htSalesCodeReSpcId;

    /**
     * 销售编码关联同包装表ID
     */
    @ApiModelProperty(name="salesCodeReSpcId",value = "销售编码关联同包装表ID")
    @Excel(name = "销售编码关联同包装表ID", height = 20, width = 30,orderNum="") 
