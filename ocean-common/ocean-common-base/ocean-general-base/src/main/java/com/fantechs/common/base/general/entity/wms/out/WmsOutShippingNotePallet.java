...
/**
 * 出货通知单明细与栈板关系表
 * wms_out_shipping_note_pallet
 * @author hyc
 * @date 2021-01-13 15:21:34
 */
@Data
@Table(name = "wms_out_shipping_note_pallet")
public class WmsOutShippingNotePallet extends ValidGroup implements Serializable {
    /**
     * 关系表ID
     */
    @ApiModelProperty(name="shippingNotePalletId",value = "关系表ID")
    @NotNull(groups = update.class,message = "关系表ID不能为空")
    @Id
    @Column(name = "shipping_note_pallet_id")
    private Long shippingNotePalletId;

    /**
     * 出货通知单明细ID
     */
    @ApiModelProperty(name="shippingNoteDetId",value = "出货通知单明细ID")
    @Excel(name = "出货通知单明细ID", height = 20, width = 30) 
